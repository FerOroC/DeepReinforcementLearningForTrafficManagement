...
class BaseKernelNetwork(object):
    ...
    * Methods for generating initial vehicle positions: Initial vehicle
    ...
    def generate_starting_positions(self, initial_config, num_vehicles=None):
        ...
        if initial_config.spacing == 'uniform':
            ...
            startpositions, startlanes = self.gen_custom_start_pos(
                initial_config, num_vehicles)
        elif initial_config.spacing == 'testBus':
            startpositions, startlanes = self.gen_testing_bus_start_pos(
                initial_config, num_vehicles)
        elif initial_config.spacing == 'London':
            startpositions, startlanes = self.gen_grid_London_start_pos(
                initial_config, num_vehicles)
        elif initial_config.spacing == 'BristolTriangle':
            startpositions, startlanes = self.gen_custom_Triangle_start_pos(
                initial_config, num_vehicles)
        else:
    ...
    def gen_testing_bus_start_pos(self, initial_config, num_vehicles):
        (x0, min_gap, bunching, lanes_distr, available_length,
         available_edges, initial_config) = \
            self._get_start_pos_util(initial_config, num_vehicles)
        if isinstance(initial_config.edges_distribution, dict):
        ...
        startpositions[0]=('gneE2',50)
        startlanes[0]=1

        startpositions[1]=('gneE4',50)
        startlanes[1]=1
    ...
    def gen_custom_Triangle_start_pos(self, initial_config, num_vehicles):
        """Generate uniformly spaced starting positions.
    ...
    def gen_grid_London_start_pos(self, initial_config, num_vehicles):
        (x0, min_gap, bunching, lanes_distr, available_length,
         available_edges, initial_config) = \
            self._get_start_pos_util(initial_config, num_vehicles)
        if isinstance(initial_config.edges_distribution, dict):
        ...
        startpositions[20]=('edge_SN_6',10)
        startlanes[20]=0

        startpositions[21]=('edge_NS_8',10)
        startlanes[21]=1
